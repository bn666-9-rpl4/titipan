...
<div class="box">
            ...
            <br>
              <center>
                <a href="tambahsuratm.php"><button type="button" class="btn btn-success">Tambah Surat Masuk</button></a> &nbsp <a href="struk/laporansuratmasuk.php?id='.$data['no_agenda'].'"><button type="button" class="btn waves-effect waves-light blue">Cetak</button></a>   
              </center>
              <br>
            </div>
            <!-- /.box-header -->
    ...
    $query = mysql_query("SELECT * FROM tb_surat_masuk INNER JOIN tb_petugas ON tb_petugas.id_petugas=tb_surat_masuk.id_petugas ORDER BY no_agenda DESC") or die(mysql_error());
    ...
    if(mysql_num_rows($query) == 0){  
      ...
      while($data = mysql_fetch_assoc($query)){ 
        ...
        echo '<tr>';
          ...
          echo '<td>'.$data['no_disposisi'].'</td>';
          echo '<td>'.$data['kepada'].'</td>';
          echo '<td>'.$data['keterangan'].'</td>';
          echo '<td>'.$data['tanggapan'].'</td>';
                  
          echo '<td><a href="editsuratm.php?id='.$data['no_agenda'].'"><button type="button" class="btn waves-effect waves-light blue">edit</button></a>  &nbsp 
          <a href="proseshapussuratm.php?id='.$data['no_agenda'].'" onclick="return confirm(\'Yakin Ingin menghapus data?\')"><button type="button" class="btn waves-effect waves-light red">Hapus</button></a></td>';
        echo '</tr>';
