...
<div class="box">
            <div class="box-header">
              <h3 class="box-title">Pemesanan</h3><br><br>
              <a href="tambahpesan.php"><button type="button" class="btn btn-success">Tambah Sewa</button></a>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
    ...
    $query = mysql_query("SELECT tb_pemesanan.*, tb_mobil.*, tb_user.* FROM tb_pemesanan INNER JOIN tb_mobil ON tb_mobil.id_mobil=tb_pemesanan.id_mobil INNER JOIN tb_user ON tb_user.id_user=tb_pemesanan.id_user") or die(mysql_error());   
    if(mysql_num_rows($query) == 0){  
      ...
      while($data = mysql_fetch_assoc($query)){ 
          ...
          echo '<td><a href="editpesan.php?id='.$data['id_pemesanan'].'"><button type="button" class="btn btn-info">edit</button></a>  &nbsp <a href="proseshapuspesan.php?id='.$data['id_pemesanan'].'" onclick="return confirm(\'Yakin Ingin menghapus data?\')"><button type="button" class="btn btn-danger">Hapus</button></a></td>';
        echo '</tr>';
